...
class phase2(Dataset):
    ...
    def __init__(self, maps=np.zeros(1), phase='train',
        ...
        self.antenna = self.data + "heatmap/var=24/20/"
        self.sample = self.data + "position/5/"
    ...
    def __getitem__(self, idx):
        ...
        sample = os.path.join(self.sample, name2)
        arr_sample = np.asarray(io.imread(sample))

        arr_mask = np.where(arr_sample > 0, 1, 0)

        target = os.path.join(self.simulation, name2)
        arr_target = np.asarray(io.imread(target))

        # To tensor
        arr_builds = self.transform(arr_build/255).type(torch.float32)

        arr_antennas = self.transform(arr_antenna/255).type(torch.float32)
        arr_sample = self.transform(arr_sample/255).type(torch.float32)

        arr_mask = self.transform(arr_mask).type(torch.float32)
        arr_targets = self.transform(arr_target/255).type(torch.float32)

        #return torch.cat([arr_builds, arr_sample], dim=0), arr_targets, name2
        return arr_builds, arr_mask, arr_targets, name2
...
#
#         return torch.cat([arr_image, arr_mask], dim=0), arr_target, name2
