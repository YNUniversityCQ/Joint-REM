...
if __name__ == '__main__':
    ...
    Radio_train = loaders.phase1(phase="train")
    Radio_val = loaders.phase1(phase="val")
    ...
    main_dataloaders = {
        'train': DataLoader(Radio_train, batch_size=16, shuffle=True, num_workers=4, pin_memory=True),
        'val': DataLoader(Radio_val, batch_size=16, shuffle=True, num_workers=4, pin_memory=True)
    }
    ...
    def train_model(model, optimizer, scheduler, num_epochs=100, targetType="dense"):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                epoch_samples = 0
                if targetType == "dense":
                    for sample, builds, target, _, name in tqdm(main_dataloaders[phase]):
                        sample, builds, target = sample.to(device), builds.to(device), target.to(device)

                        optimizer.zero_grad()

                        with torch.set_grad_enabled(phase == 'train'):

                            outputs = model(sample, builds)
    ...
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=190, gamma=0.1)
